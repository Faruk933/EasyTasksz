...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (status === "approved") {
      const payoutAmount = Number(withdrawal.payout_amount ?? withdrawal.amount);
      ...
      if (!oxapayResponse.ok || oxapayResult.error) {
        ...
        await supabase
          .from("withdrawals")
          .update({ payout_status: "failed", payout_error: errorMsg })
          .eq("id", withdrawalId);

        return new Response(JSON.stringify({ error: "OxaPay payout failed: " + errorMsg }), {
          status: 400,
          headers: corsHeaders,
        });
      }
      ...
      return new Response(JSON.stringify({ success: true, trackId, payoutStatus }), {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
