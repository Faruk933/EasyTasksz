...
const POSTBACK_SECRET=Deno.env.get("MONETAG_POSTBACK_SECRET")?.trim()||"";
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(!POSTBACK_SECRET||!timingSafeEqualText(supplied,POSTBACK_SECRET)) return new Response("Unauthorized",{status:401});
    ...
    const {error:updateError}=await s.from("monetag_ad_rewards").update({
      status:"valued",
      valued_at:new Date().toISOString(),
      estimated_price:Number.isFinite(price)&&price>0?price:0
    }).eq("ymid",ymid).eq("status","pending");
