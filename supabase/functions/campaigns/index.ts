...
Deno.serve(async (req) => {
  ...
  try {
    const { initData, action, title, message, targetType, campaignType, isActive, bonusEnabled, bonusAmount, campaignId } = await req.json();
    ...
    if (action === "create") {
      ...
      const type = campaignType === "welcome" ? "welcome" : "one_time";
      ...
      if (type === "one_time" && !['all', 'new'].includes(targetType)) return response({ error: "Invalid target" }, 400);

      const { data: campaign, error } = await db.from("campaigns").insert({ title: cleanTitle, message: cleanMessage, target_type: type === "welcome" ? "new" : targetType, campaign_type: type, is_active: type === "welcome" ? Boolean(isActive) : false, bonus_enabled: Boolean(bonusEnabled), bonus_amount: amount }).select().single();
      if (error) {
        if (error.code === "23505") return response({ error: "There is already an active welcome campaign. Deactivate it before activating another." }, 409);
        throw error;
      }
    ...
    if (action === "process") {
      ...
      for (const record of records || []) {
        ...
        if (campaign.bonus_enabled && Number(campaign.bonus_amount) > 0 && !bonusGiven) {
          ...
          if (claimed) {
            ...
            await db.from("transactions").insert({ user_id: record.user_id, type: "bonus", amount, description: `Campaign Bonus: ${campaign.title}` });
        ...
        try {
          ...
          await db.from("campaign_user_records").update({ notification_sent: true, notification_error: null }).eq("id", record.id); sent++;
        } catch (err) { await db.from("campaign_user_records").update({ notification_error: String(err) }).eq("id", record.id); failed++; }
      }
      ...
      const { count: total } = await db.from("campaign_user_records").select("id", { count: "exact", head: true }).eq("campaign_id", id);
