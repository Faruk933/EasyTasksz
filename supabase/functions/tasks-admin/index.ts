    ...
    if (action === "review-submission") {
      ...
      if (status === "approved") {
        ...
        if (user) {
          ...
          const newBalance = Number(user.balance ?? 0) + rewardAmt;
          const newTotalEarned = Number(user.total_earned ?? 0) + rewardAmt;
          ...
          if (user.referred_by) {
            const commission = rewardAmt * (commissionPercent / 100);
