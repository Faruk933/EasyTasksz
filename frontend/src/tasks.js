async function callTasks(payload) {
  ...
  const response = await fetch("https://iewdxruivjwblsnsjicq.supabase.co/functions/v1/tasks", {
    method: "POST",
...
export function startOfferTask(taskId) { return callTasks({ action: "start-offer-task", taskId }); }
