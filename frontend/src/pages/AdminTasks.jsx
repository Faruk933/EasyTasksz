...
export default function AdminTasks() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setTitle(""); setInstructions(""); setRewardAmount(""); setTaskUrl(""); setShowForm(false); loadTasks();
    ...
    <div style={{ padding: 16 }}>
      ...
      {tasks.length === 0 ? <p style={{ color: "#94a3b8", textAlign: "center" }}>No tasks yet</p> : tasks.map((task) => (
        <div key={task.id} className="wallet-card" style={{ marginBottom: 12 }}>
          ...
          <button className="wallet-btn" onClick={() => handleToggleActive(task)} style={{ marginTop: 8, background: task.is_active ? "#7f1d1d" : "#166534" }}>{task.is_active ? "Deactivate" : "Activate"}</button>
        </div>
      ))}
    </div>
  );
