...
import { createCampaign, deactivateCampaign, listCampaigns, previewCampaign, processCampaign } from "../campaigns";
...
export default function AdminCampaigns() {
  ...
  async function deactivate(id) {
    if (!window.confirm("Deactivate this welcome campaign? New users will stop receiving this campaign. You can then activate a new welcome campaign.")) return;
    setBusy(true); setError("");
    try { await deactivateCampaign(id); await refresh(); alert("Welcome campaign deactivated. You can now activate another one."); } catch (e) { setError(e.message); } finally { setBusy(false); }
  }
  return <div style={{ padding: 16 }}>
    ...
    {history.length === 0 ? <p style={{ color: "#94a3b8" }}>No campaigns yet.</p> : history.map((c) => <div key={c.id} style={{ background: "#1e293b", borderRadius: 12, padding: 12, marginBottom: 10 }}><strong>{c.title}</strong><div style={{ fontSize: 12, color: "#94a3b8", marginTop: 5 }}>{c.campaign_type === "welcome" ? `🎉 Welcome · ${c.is_active ? "ACTIVE" : "inactive"}` : `📢 One-time · Recipients: ${c.recipients}`} · Sent: {c.notificationsSent} · Failed: {c.failed} · Bonus: ${Number(c.bonusDistributed).toFixed(2)}</div><div style={{ fontSize: 12, color: "#94a3b8" }}>{new Date(c.created_at).toLocaleString()}</div>{c.campaign_type === "one_time" && c.notificationsSent < c.recipients && <button disabled={busy} onClick={() => resume(c.id)} style={{ ...buttonStyle, marginTop: 8 }}>Resume</button>}{c.campaign_type === "welcome" && c.is_active && <button disabled={busy} onClick={() => deactivate(c.id)} style={{ ...deactivateButtonStyle, marginTop: 8 }}>Deactivate Welcome Campaign</button>}{c.campaign_type === "welcome" && c.test_recipients > 0 && c.test_notifications_sent === 0 && <button disabled={busy} onClick={() => resume(c.id)} style={{ ...buttonStyle, marginTop: 8 }}>Send Test to Test User</button>}</div>)}
  </div>;
}
...
const deactivateButtonStyle = { width: "100%", padding: 12, border: 0, borderRadius: 10, background: "#dc2626", color: "white", fontWeight: "bold" };
