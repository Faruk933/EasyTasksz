...
export default function AdminCampaigns() {
  ...
  async function sendWelcomeTest(id) { setBusy(true); setError(""); try { const result = await processCampaign(id); setProgress(result); await refresh(); alert("Welcome campaign test processed."); } catch (e) { setError(e.message); } finally { setBusy(false); } }
  ...
  return <div style={{ padding: 16 }}>
    ...
    <div style={{ background: "#1e293b", borderRadius: 14, padding: 14 }}>
      ...
      {form.campaignType === "welcome" && <><div style={{ background: "#0f172a", borderRadius: 10, padding: 10, marginBottom: 12, color: "#cbd5e1", fontSize: 13 }}>Future users only. This will not automatically send to existing users. Only one active welcome campaign is allowed.</div><label>Test User (optional)</label><input value={form.testUserId} onChange={(e) => setForm({ ...form, testUserId: e.target.value })} placeholder="Telegram ID e.g. 1115177381" inputMode="numeric" style={inputStyle} /><p style={{ color: "#94a3b8", fontSize: 12 }}>Your test user can be processed from Campaign History without changing who receives the automatic welcome campaign.</p></>}
    ...
    {history.length === 0 ? <p style={{ color: "#94a3b8" }}>No campaigns yet.</p> : history.map((c) => <div key={c.id} style={{ background: "#1e293b", borderRadius: 12, padding: 12, marginBottom: 10 }}><strong>{c.title}</strong><div style={{ fontSize: 12, color: "#94a3b8", marginTop: 5 }}>{c.campaign_type === "welcome" ? `🎉 Welcome · ${c.is_active ? "ACTIVE" : "inactive"}` : `📢 One-time · Recipients: ${c.recipients}`} · Sent: {c.notificationsSent} · Failed: {c.failed} · Bonus: ${Number(c.bonusDistributed).toFixed(2)}</div><div style={{ fontSize: 12, color: "#94a3b8" }}>{new Date(c.created_at).toLocaleString()}</div>{c.campaign_type === "welcome" && c.test_recipients > 0 && c.test_notifications_sent === 0 && <button disabled={busy} onClick={() => sendWelcomeTest(c.id)} style={{ ...buttonStyle, marginTop: 8 }}>Send Test to Test User</button>}{c.campaign_type === "one_time" && c.notificationsSent < c.recipients && <button disabled={busy} onClick={() => resume(c.id)} style={{ ...buttonStyle, marginTop: 8 }}>Resume</button>}</div>)}
  </div>;
}
