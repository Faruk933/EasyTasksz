...
const initialForm = { title: "", message: "", campaignType: "one_time", targetType: "all", bonusEnabled: true, bonusAmount: "3.00", isActive: true };
...
export default function AdminCampaigns() {
  ...
  async function updateTarget(targetType) {
    ...
    try { setRecipientCount((await previewCampaign(targetType, form.campaignType)).recipients || 0); } catch (e) { setError(e.message); }
  }

  useEffect(() => { previewCampaign(form.targetType, form.campaignType).then((r) => setRecipientCount(r.recipients || 0)).catch(() => {}); }, [form.campaignType, form.targetType]);
  ...
  async function send() {
    ...
    const isWelcome = form.campaignType === "welcome";
    const confirmed = window.confirm(isWelcome
      ? `Activate this Welcome Campaign for future new users?\n\nBonus: $${amount.toFixed(2)}\n\nEach new user can receive it only once.`
      : `You are about to send this campaign to ${recipientCount.toLocaleString()} users.\n\nBonus: $${amount.toFixed(2)}\n\nContinue?`);
    if (!confirmed) return;
    ...
    try {
      ...
      if (!isWelcome) {
        ...
        setProgress(result);
        while (!result.done) { result = await processCampaign(created.campaign.id); setProgress(result); }
      }
      setForm(initialForm);
      await refresh();
      alert(isWelcome ? "Welcome campaign activated. Future new users will receive it once." : "Campaign completed.");
  ...
  return <div style={{ padding: 16 }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
      <div><h1 style={{ margin: 0 }}>Notifications & Campaigns</h1><p style={{ color: "#94a3b8" }}>Send reusable Telegram notifications or automate a welcome bonus.</p></div>
    ...
    <div style={{ background: "#1e293b", borderRadius: 14, padding: 14 }}>
      ...
      <select value={form.campaignType} onChange={(e) => setForm({ ...form, campaignType: e.target.value })} style={inputStyle}>
        ...
        <option value="welcome">🎉 Automatic welcome campaign</option>
      </select>
      ...
      {form.campaignType === "one_time" && <><label>Target Users</label><select value={form.targetType} onChange={(e) => updateTarget(e.target.value)} style={inputStyle}><option value="all">All users</option><option value="new">New users (last 7 days)</option><option value="not_received">Users who have not received this campaign</option></select><p style={{ color: "#94a3b8", fontSize: 12 }}>Recipients: {recipientCount.toLocaleString()}</p></>}
      {form.campaignType === "welcome" && <div style={{ background: "#0f172a", borderRadius: 10, padding: 10, marginBottom: 12, color: "#cbd5e1", fontSize: 13 }}>Future users only. This will not send anything to existing users. Only one active welcome campaign is allowed.</div>}
    ...
    {history.length === 0 ? <p style={{ color: "#94a3b8" }}>No campaigns yet.</p> : history.map((c) => <div key={c.id} style={{ background: "#1e293b", borderRadius: 12, padding: 12, marginBottom: 10 }}><strong>{c.title}</strong><div style={{ fontSize: 12, color: "#94a3b8", marginTop: 5 }}>{c.campaign_type === "welcome" ? `🎉 Welcome · ${c.is_active ? "ACTIVE" : "inactive"}` : `📢 One-time · Recipients: ${c.recipients}`} · Sent: {c.notificationsSent} · Failed: {c.failed} · Bonus: ${Number(c.bonusDistributed).toFixed(2)}</div><div style={{ fontSize: 12, color: "#94a3b8" }}>{new Date(c.created_at).toLocaleString()}</div>{c.campaign_type === "one_time" && c.notificationsSent < c.recipients && <button disabled={busy} onClick={() => resume(c.id)} style={{ ...buttonStyle, marginTop: 8 }}>Resume</button>}</div>)}
  </div>;
}
