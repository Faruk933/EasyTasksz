async function callTasksAdmin(payload) {
  ...
  const response = await fetch("https://iewdxruivjwblsnsjicq.supabase.co/functions/v1/tasks-admin", {
    method: "POST",
...
export function createTask(title, instructions, rewardAmount, taskUrl, provider, offerId) { return callTasksAdmin({ action: "create-task", title, instructions, rewardAmount, taskUrl, provider, offerId }); }
